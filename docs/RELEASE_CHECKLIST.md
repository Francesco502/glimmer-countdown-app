...
- 重复日期边界补充（2026-07-17）：聚焦 JVM 用例先复现 2020-02-29 年度事件在 2025-02-27 被错误反推为 2024-02-28；修复后同一测试类 12/12 通过，并覆盖 31 日月度、31 日半年和闰日年度重复的原始日期锚点。尚未完成全部公历/农历周期、跨年及运行态提醒矩阵，因此重复规则综合项仍保留未勾选。
- 最终质量门补充（2026-07-17）：当前 HEAD 的 Direct / Play JVM 各 413 项通过，均为 0 failures / 0 errors / 0 skipped；API 37 connected 10/10 通过，包含完整 6→10 迁移、编辑恢复、筛选拖拽、小组件多实例及 Compose 语义回归。`compileDirectDebugAndroidTestKotlin`、Direct / Play Debug 构建与五项 lint / vital lint 同轮成功，DirectDebug、DirectRelease、PlayRelease 报告均为 `No issues found.`
