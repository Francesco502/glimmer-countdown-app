# GitHub 提交与发布流程（v3.12）

本文档用于当前 `3.12` 版本的代码提交、推送、标签与 GitHub Release 操作。
...
```bash
git tag -fa v3.12 -m "Release v3.12"
git push origin v3.12 --force
```
...
- 自动从 `CHANGELOG.md` 提取 `3.12` 小节作为 Release Notes
- 如果 Release 已存在，会自动更新说明
- 如果同名 APK / AAB 资源已存在，会自动删除旧资源并上传新 APK / AAB

## 6. 发布后核对

- Release 标题、标签与说明是否对应 `v3.12`
- 上传的 APK 文件名是否为 `glimmer-countdown-3-12.apk`
