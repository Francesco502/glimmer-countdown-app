...
2. **Choose a tag**：新建标签，**必须与当前版本号一致**，例如当前 `VERSION_NAME=3.1` 则填 **`v3.1`**（带前缀 `v`）。选 “Create new tag” 后发布。
3. **Release title**：如 `v3.1`。
...
5. **Attach binaries**：将本地构建好的 APK 拖拽上传。  
   - 路径示例：`app\build\outputs\apk\direct\release\glimmer-countdown-3-1.apk`（随 VERSION_NAME 变化）  
