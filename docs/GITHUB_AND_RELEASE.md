# GitHub 提交与发布流程（v3.16）

本文档用于当前 `3.16` 版本的代码提交、推送、标签与 GitHub Release 操作。
...
```bash
git tag -fa v3.16 -m "Release v3.16"
git push origin v3.16 --force
```
...
- GitHub Release：`app/build/outputs/apk/direct/release/glimmer-countdown-3-16.apk`
...
- 自动从 `CHANGELOG.md` 提取 `3.16` 小节作为 Release Notes
...
- Release 标题、标签与说明是否对应 `v3.16`
- 上传的 APK 文件名是否为 `glimmer-countdown-3-16.apk`
- Release 资产中没有 `app-play-release.apk` 或 `app-play-release.aab`
- Direct APK `versionName` 是否为 `3.16`
- Play APK / AAB `versionName` 是否为 `3.16-play`
- Play APK 是否不包含 `REQUEST_INSTALL_PACKAGES`
- 抽检首页右上近期入口、月历选中日期内容与年月选择、详情轻量主卡与分享卡、新建 / 编辑提醒滚轮、设置页样张、小组件配置、启动页和 Direct 渠道检查更新
